"""Server configuration from environment variables (prefix ``BOX_SERVER_``).

In production the variables come from ``/etc/box-server/box-server.env`` via systemd's
...
class Settings(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="BOX_SERVER_", extra="ignore")
    ...
    data_dir: Path = Path("/var/lib/box-server")
    ...
    session_cookie_name: str = "box_session"
    ...
    mail_from: str = "Box <box@localhost>"
